...
export function decideEntry(state:Office,id:string,decision:'Confirmada'|'Rejeitada',reason:string,actor:string,date:string):Office{
 ...
 return {...state,entries:state.entries.map(e=>e.id===id?{...e,status:decision,reason:decision==='Rejeitada'?reason.trim():'',reference:decision==='Confirmada'?`MAT-${e.id.replace('PRE-DEMO-','')}`:'',history:[...e.history,{action:decision==='Confirmada'?'Confirmação simulada':`Rejeição simulada: ${reason.trim()}`,actor,date}]}:e)};
}
